...
// src/cellselection.ts
var CellSelection = class extends import_prosemirror_state2.Selection {
...
function selectedRect(state) {
  ...
  return __spreadProps(__spreadValues({}, rect), { tableStart, map, table });
}
...
function mergeCells(state, dispatch) {
  ...
  if (dispatch) {
    ...
    tr.setNodeMarkup(mergedPos + rect.tableStart, null, __spreadProps(__spreadValues({}, addColSpan(
      mergedCell.attrs,
      ...
      if (state.selection instanceof CellSelection)
        state.selection.forEachCell((node, pos) => {
          if (node.attrs[name] !== value)
            tr.setNodeMarkup(pos, null, __spreadProps(__spreadValues({}, node.attrs), {
              [name]: value
            }));
        });
      else
